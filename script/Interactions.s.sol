...
contract FundSubscription is Script, CodeConstants {
    ...
    function fundSubscriptionUsingConfig() public {
        ...
        fundSubscription(vrfCoordinator, subsId, linkToken, helperConfig.getConfig().account);
    }

    function fundSubscription(
        address vrfCoordinator, uint256 subsId, address linkToken, address account
    ) public {
        ...
        if (block.chainid == LOCAL_CHAIN_ID) {
            vm.startBroadcast();
            VRFCoordinatorV2_5Mock(vrfCoordinator).fundSubscription(subsId, FUND_AMOUNT * 100);
            vm.stopBroadcast();
        } else {
            vm.startBroadcast(account);
...
contract AddConsumer is Script {
    function addConsumerUsingConfig(address mostRecentlyDeployed) public {
        ...
        address vrfCoordinator = helperConfig.getConfig().vrfCoordinator;
        uint256 subsId = helperConfig.getConfig().subscriptionId;
        addConsumer(mostRecentlyDeployed, vrfCoordinator, subsId, helperConfig.getConfig().account);
    }

    function addConsumer(
        address contractToAddToVrf, address vrfCoordinator, uint256 subsId, address account
    ) public {
